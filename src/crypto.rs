...
fn getrandom(buf: &mut [u8]) -> Result<(), &'static str> {
    ...
    f.read_exact(buf)
        .map_err(|_| "Error leyendo /dev/urandom")?;
    Ok(())
}
...
fn qr(a: u32, b: u32, c: u32, d: u32) -> (u32, u32, u32, u32) {
    ...
    let d2 = (d ^ a2).rotate_left(16);
    let c2 = c.wrapping_add(d2);
    let b2 = (b ^ c2).rotate_left(12);
    let a3 = a2.wrapping_add(b2);
    let d3 = (d2 ^ a3).rotate_left(8);
    let c3 = c2.wrapping_add(d3);
    let b3 = (b2 ^ c3).rotate_left(7);
    (a3, b3, c3, d3)
}
...
fn chacha20_block(key: &[u8; 32], nonce: &[u8; 12], counter: u32) -> [u8; 64] {
    ...
    for _ in 0..10 {
        let mut t;
        t = qr(s[0], s[4], s[8], s[12]);
        s[0] = t.0;
        s[4] = t.1;
        s[8] = t.2;
        s[12] = t.3;
        t = qr(s[1], s[5], s[9], s[13]);
        s[1] = t.0;
        s[5] = t.1;
        s[9] = t.2;
        s[13] = t.3;
        t = qr(s[2], s[6], s[10], s[14]);
        s[2] = t.0;
        s[6] = t.1;
        s[10] = t.2;
        s[14] = t.3;
        t = qr(s[3], s[7], s[11], s[15]);
        s[3] = t.0;
        s[7] = t.1;
        s[11] = t.2;
        s[15] = t.3;
        t = qr(s[0], s[5], s[10], s[15]);
        s[0] = t.0;
        s[5] = t.1;
        s[10] = t.2;
        s[15] = t.3;
        t = qr(s[1], s[6], s[11], s[12]);
        s[1] = t.0;
        s[6] = t.1;
        s[11] = t.2;
        s[12] = t.3;
        t = qr(s[2], s[7], s[8], s[13]);
        s[2] = t.0;
        s[7] = t.1;
        s[8] = t.2;
        s[13] = t.3;
        t = qr(s[3], s[4], s[9], s[14]);
        s[3] = t.0;
        s[4] = t.1;
        s[9] = t.2;
        s[14] = t.3;
...
mod tests {
    ...
    fn test_poly1305_known() {
        ...
        let key: [u8; 32] = [
            0xdc, 0x97, 0xfc, 0xb3, 0x5e, 0x28, 0xbf, 0x02, 0x2e, 0x0f, 0x8a, 0x3f, 0xb3, 0x0f,
            0x92, 0x85, 0xc9, 0x32, 0x56, 0x05, 0x4f, 0x2d, 0x09, 0xf1, 0x37, 0x9a, 0xbc, 0xea,
            0x43, 0xb6, 0x10, 0x39,
        ];

        // empty
        assert_eq!(
            hex_encode(&poly1305_mac(&key, b"")),
            "c93256054f2d09f1379abcea43b61039"
        );
        // 5 bytes
        assert_eq!(
            hex_encode(&poly1305_mac(&key, b"Hello")),
            "724a8babd1c756b143e551a9a6221696"
        );
        // 16 bytes
        assert_eq!(
            hex_encode(&poly1305_mac(&key, &[b'A'; 16])),
            "ad5ef76e4f5d1363129a12ebdfca02e1"
        );
        // 32 bytes
        assert_eq!(
            hex_encode(&poly1305_mac(&key, &[b'A'; 32])),
            "a3767d22e24bff460da9be377fbb7aee"
        );
        // 33 bytes (partial block)
        assert_eq!(
            hex_encode(&poly1305_mac(&key, &[b'A'; 33])),
            "8f614e60aaab74afb3d96bd7cd84e2dc"
        );
    }
}

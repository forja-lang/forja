...
impl DiagramGenerator {
    ...
    fn add_node(&mut self, label: &str, shape_start: &str, shape_end: &str) -> String {
        ...
        if label.is_empty() {
            self.output.push_str(&format!("    {id}{shape_start}{shape_end}\n"));
        } else {
            let clean_label = label
                ...
                .replace('<', "&lt;")
                .replace('>', "&gt;");
            self.output.push_str(&format!("    {id}{shape_start}\"{clean_label}\"{shape_end}\n"));
        }
        id
    }
    ...
    fn gen_block(&mut self, decls: &[Declaracion], _parent: &str) -> (Option<String>, Option<String>) {
    ...
    fn gen_decl(&mut self, decl: &Declaracion) -> (Option<String>, Option<String>) {
        match decl {
            ...
            Declaracion::AsignacionMiembro { objeto, miembro, valor, .. } => {
                let node = self.add_node(&format!("{}.{miembro} = {}", self.ec(objeto), self.ec(valor)), "[", "]");
                (Some(node.clone()), Some(node))
            }
            Declaracion::AsignacionIndex { nombre, indice, valor, .. } => {
                let node = self.add_node(&format!("{nombre}[{}] = {}", self.ec(indice), self.ec(valor)), "[", "]");
                (Some(node.clone()), Some(node))
            }
            ...
            Declaracion::Clase { nombre, metodos, .. } => {
                ...
                for m in metodos {
                    ...
                    self.output.push_str(&format!("        subgraph Met_{}_{} [\"metodo {}({})\"]\n", nombre, m.nombre, m.nombre, ps.join(", ")));
            ...
            Declaracion::Si { condicion, bloque_verdadero, bloque_falso } => {
                ...
                if let Some(ss) = si_start {
                    self.output.push_str(&format!("    {} -- Si --> {}\n", cond_node, ss));
                }
                ...
                let sino_exit = if let Some(bf) = bloque_falso {
                    ...
                    if let Some(sn) = sino_start {
                        self.output.push_str(&format!("    {} -- Sino --> {}\n", cond_node, sn));
                    }
            ...
            Declaracion::Mientras { condicion, bloque } => {
                ...
                self.output.push_str(&format!("    {} -- No --> {}\n", cond_node, exit_node));
            ...
            Declaracion::Repetir { cantidad, bloque } => {
                ...
                if let Some(bs) = body_start {
                    self.output.push_str(&format!("    {} -- Bucle --> {}\n", cond_node, bs));
                }
                if let Some(be) = body_end {
                    self.output.push_str(&format!("    {} --> {}\n", be, cond_node));
                } else {
                ...
                self.output.push_str(&format!("    {} -- Fin --> {}\n", cond_node, exit_node));
            ...
            Declaracion::Implementacion { rasgo_nombre, clase_nombre, metodos } => {
                ...
                for m in metodos {
                    let ps: Vec<String> = m.parametros.iter().map(|p| p.nombre.clone()).collect();
                    self.output.push_str(&format!("        subgraph ImplMet_{} [\"metodo {}({})\"]\n", m.nombre, m.nombre, ps.join(", ")));
                    let m_start = self.add_node("Inicio", "([", "])");
                    let (m_body_start, m_body_end) = self.gen_block(&m.cuerpo, &m.nombre);
                    if let Some(mbs) = m_body_start {
                        self.output.push_str(&format!("        {} --> {}\n", m_start, mbs));
            ...
            Declaracion::AsignacionMultiple { variables, valor, .. } => {
                let node = self.add_node(&format!("{} = {}", variables.join(", "), self.ec(valor)), "[", "]");
                (Some(node.clone()), Some(node))
            }
        }
    }
    ...
    fn ec(&self, e: &Expresion) -> String {
        match e {
            ...
            Expresion::Referencia { expr: ex, mutable } => {
                if *mutable { format!("&mut {}", self.ec(ex)) } else { format!("&{}", self.ec(ex)) }
            }
            ...
            Expresion::Mapa(pares) => {
                let entries: Vec<String> = pares.iter().map(|(k, v)| format!("{}: {}", self.ec(k), self.ec(v))).collect();
                format!("{{{}}}", entries.join(", "))
            }
    ...
    fn dc(&self, d: &Declaracion) -> String {
        match d {
            Declaracion::Variable { nombre, valor, .. } => {
                if let Some(v) = valor { format!("{nombre} = {}", self.ec(v)) } else { nombre.clone() }
            }
            Declaracion::Asignacion { nombre, valor, .. } => format!("{nombre} = {}", self.ec(valor)),
            _ => "?".to_string()
        }
    }

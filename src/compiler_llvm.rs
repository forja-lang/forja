...
impl LlvmBackend {
    ...
    pub fn compile(&mut self, decls: &[Declaracion]) -> Result<(), String> {
        for d in decls {
            ...
            {
                let ps: Vec<String> = (0..parametros.len())
    ...
    fn lb(&mut self, p: &str) -> String {
        let n = self.lc;
        self.lc += 1;
        format!("%{}{}", p, n)
    }
    fn sl(&mut self) -> String {
        let n = self.sc;
        self.sc += 1;
        format!(".str.{}", n)
    }
    ...
    fn store(&mut self, ptr: &str, val: &str) {
        line!(self.out, "store i64 {}, i64* {}", val, ptr);
    }
    ...
    fn recolectar_anterior_temps(&mut self, expr: &Expresion) -> Result<(), String> {
        match expr {
                ...
                let val_reg = match inner.as_ref() {
                    Expresion::Identificador { nombre: name, .. } => {
                        if let Some(p) = self.vars.get(name).cloned() {
                            ...
                            line!(
                                self.out,
                                "{} = load i64, i64* {} ; anterior snapshot of {}",
                                r,
                                p,
                                name
                            );
            ...
            Expresion::Instanciacion { argumentos, .. } => {
                for a in argumentos {
                    self.recolectar_anterior_temps(a)?;
                }
                Ok(())
            }
    ...
    fn generar_expr_con_resultado(
        ...
        match expr {
            ...
            Expresion::LiteralDecimal(d) => {
                let r = self.r();
                line!(self.out, "{} = fadd double 0.0, {:e}", r, d);
                let i = self.r();
                line!(self.out, "{} = bitcast double {} to i64", i, r);
                Ok(i)
            }
            Expresion::LiteralTexto(s) => {
                ...
                let i = self.r();
                line!(self.out, "{} = ptrtoint i8* {} to i64", i, r);
                Ok(i)
            }
            ...
            Expresion::Identificador { nombre: n, .. } => {
                if n == "verdadero" {
                    Ok("1".into())
                } else if n == "falso" || n == "nulo" {
                    Ok("0".into())
                } else if let Some(r) = self.load(n) {
                    Ok(r)
                } else if self.funcs.contains(n) {
                    let r = self.r();
                    line!(self.out, "{} = call i64 @{}()", r, n);
                    Ok(r)
                } else {
                    Ok("0".into())
                }
            ...
            Expresion::Binaria {
                ...
                match operador {
                    ...
                    Operador::Mayor => {
                        let c = self.r();
                        line!(self.out, "{} = icmp sgt i64 {}, {}", c, lv, rv);
                        line!(self.out, "{} = zext i1 {} to i64", reg, c);
                    }
                    Operador::Menor => {
                        let c = self.r();
                        line!(self.out, "{} = icmp slt i64 {}, {}", c, lv, rv);
                        line!(self.out, "{} = zext i1 {} to i64", reg, c);
                    }
                    Operador::MayorIgual => {
                        let c = self.r();
                        line!(self.out, "{} = icmp sge i64 {}, {}", c, lv, rv);
                        line!(self.out, "{} = zext i1 {} to i64", reg, c);
                    }
                    Operador::MenorIgual => {
                        let c = self.r();
                        line!(self.out, "{} = icmp sle i64 {}, {}", c, lv, rv);
                        line!(self.out, "{} = zext i1 {} to i64", reg, c);
                    }
                    Operador::IgualIgual => {
                        let c = self.r();
                        line!(self.out, "{} = icmp eq i64 {}, {}", c, lv, rv);
                        line!(self.out, "{} = zext i1 {} to i64", reg, c);
                    }
                    Operador::Diferente => {
                        let c = self.r();
                        line!(self.out, "{} = icmp ne i64 {}, {}", c, lv, rv);
                        line!(self.out, "{} = zext i1 {} to i64", reg, c);
                    }
                    Operador::Y => {
                        let zl = self.r();
            ...
            Expresion::Unaria { operador, expr: e } => {
                ...
                match operador {
                    OperadorUnario::Negar => {
                        let r = self.r();
                        line!(self.out, "{} = sub i64 0, {}", r, v);
                        Ok(r)
                    }
                    OperadorUnario::No => {
                        let r = self.r();
                        line!(self.out, "{} = icmp eq i64 {}, 0", r, v);
                        let x = self.r();
                        line!(self.out, "{} = zext i1 {} to i64", x, r);
                        Ok(x)
                    }
                }
            }
            ...
            Expresion::LlamadaFuncion { nombre, argumentos } => {
                ...
                for a in argumentos {
                    rs.push(format!(
                        "i64 {}",
                        self.generar_expr_con_resultado(a, ret_val)?
                    ));
                }
                let s = rs.join(", ");
                let r = self.r();
            ...
            Expresion::AccesoMiembro { objeto, miembro } => {
                ...
                let p = self.r();
                line!(self.out, "{} = inttoptr i64 {} to i8*", p, o);
                let g = self.r();
                line!(self.out, "{} = getelementptr i8, i8* {}, i64 0", g, p);
                let ip = self.r();
                line!(self.out, "{} = bitcast i8* {} to i64*", ip, g);
                let l = self.r();
                line!(
                    self.out,
                    "{} = load i64, i64* {} ; miembro {}",
                    l,
                    ip,
                    miembro
                );
            ...
            Expresion::Index { objeto, indice } => {
                ...
                let p = self.r();
                line!(self.out, "{} = inttoptr i64 {} to i64*", p, o);
                let e = self.r();
                line!(self.out, "{} = getelementptr i64, i64* {}, i64 {}", e, p, i);
                let l = self.r();
                line!(self.out, "{} = load i64, i64* {}", l, e);
                Ok(l)
            }
            Expresion::Referencia { expr: e, .. } => self.generar_expr_con_resultado(e, ret_val),
            Expresion::Arreglo(elementos) => {
                for e in elementos {
                    self.generar_expr_con_resultado(e, ret_val)?;
                }
                Ok("0".into())
            }
            Expresion::Coincidir { expr: e, brazos } => {
    ...
    fn decl(&mut self, d: &Declaracion) -> Result<(), String> {
        match d {
            ...
            Declaracion::Asignacion { nombre, valor, .. } => {
                if let Some(p) = self.vars.get(nombre).cloned() {
                    let r = self.expr(valor)?;
                    self.store(&p, &r);
                }
            }
            ...
            Declaracion::Retornar { valor, .. } => {
                if self.postcondiciones_activas {
                    ...
                    match valor {
                        Some(v) => {
                            let r = self.expr(v)?;
                            line!(self.out, "ret i64 {}", r);
                        }
                        None => line!(self.out, "ret i64 0"),
                    }
                }
            }
    ...
    fn funcion(
        ...
        if has_post {
            ...
            line!(
                self.out,
                "{} = load i64, i64* {} ; retval for postcondiciones",
                ret_reg,
                self.retval_ptr.as_ref().unwrap()
            );
    ...
    fn para(
        &mut self,
        init: Option<&Declaracion>,
        cond: Option<&Expresion>,
        inc: Option<&Declaracion>,
        body: &[Declaracion],
    ) -> Result<(), String> {
        if let Some(i) = init {
            self.decl(i)?;
        ...
        if let Some(cv) = cond {
            ...
            line!(self.out, "br label %{}", b);
        }
        raw!(self.out, "{}:", b);
        for d in body {
            self.decl(d)?;
        }
        line!(self.out, "br label %{}", n);
        raw!(self.out, "{}:", n);
        if let Some(i) = inc {
            self.decl(i)?;
        }
    ...
    fn repetir(&mut self, cant: &Expresion, body: &[Declaracion]) -> Result<(), String> {
        ...
        raw!(self.out, "{}:", c);
        let cn = self.r();
        line!(self.out, "{} = load i64, i64* {}", cn, cp);
        let cmp = self.r();
        line!(self.out, "{} = icmp slt i64 {}, {}", cmp, cn, t);
    ...
    fn llamar(
        ...
        if name == "escribir" {
            return self.escribir(args, ret);
        }
        let mut rs = Vec::new();
        for a in args {
            rs.push(format!("i64 {}", self.expr(a)?));
        }
        let s = rs.join(", ");
        if ret {
            let r = self.r();
            line!(self.out, "{} = call i64 @{}({})", r, name, s);
            Ok(Some(r))
        } else {
            line!(self.out, "call i64 @{}({})", name, s);
            Ok(None)
        }
    }
    ...
    fn escribir(&mut self, args: &[Expresion], _ret: bool) -> Result<Option<String>, String> {
        ...
        line!(
            self.out,
            "{} = getelementptr [2 x i8], [2 x i8]* @{}, i64 0, i64 0",
            nr,
            nl
        );
    ...
    fn expr(&mut self, e: &Expresion) -> Result<String, String> {
        match e {
            ...
            Expresion::Binaria {
                ...
                operador,
                derecha,
            } => self.bin(izquierda, operador, derecha),
            ...
            Expresion::LlamadaFuncion { nombre, argumentos } => Ok(self
                .llamar(nombre, argumentos, true)?
            ...
            Expresion::Instanciacion { argumentos, .. } => {
                ...
                let r = self.r();
                line!(self.out, "{} = call i8* @malloc(i64 {})", r, sz);
                let ptr = self.r();
                line!(self.out, "{} = ptrtoint i8* {} to i64", ptr, r);
                for (i, arg) in argumentos.iter().enumerate() {
                    let v = self.expr(arg)?;
                    let ep = self.r();
                    line!(self.out, "{} = inttoptr i64 {} to i8*", ep, ptr);
                    let gp = self.r();
                    line!(
                        self.out,
                    ...
                    let ip = self.r();
                    line!(self.out, "{} = bitcast i8* {} to i64*", ip, gp);
            ...
            Expresion::Referencia { expr: ex, .. } => {
                if let Expresion::Identificador { nombre: n, .. } = ex.as_ref() {
                    if let Some(p) = self.vars.get(n).cloned() {
                        let r = self.r();
                        line!(self.out, "{} = ptrtoint i64* {} to i64", r, p);
                        return Ok(r);
                    }
                }
                self.expr(ex)
            ...
            Expresion::Arreglo(elementos) => {
                ...
                line!(self.out, "{} = call i8* @malloc(i64 {})", r, sz);
                let ip = self.r();
                line!(self.out, "{} = bitcast i8* {} to i64*", ip, r);
                ...
                let ret = self.r();
                line!(self.out, "{} = ptrtoint i64* {} to i64", ret, ip);
                Ok(ret)
            }
            Expresion::Index { objeto, indice } => {
                let o = self.expr(objeto)?;

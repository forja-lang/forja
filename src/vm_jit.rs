...
const OP_PUSH_NULO: u8 = 4;    const OP_POP: u8 = 5;
const OP_DUP: u8 = 6;          const OP_LOAD: u8 = 7;
const OP_STORE: u8 = 8;        const OP_DECLARE: u8 = 9;
const OP_ADD: u8 = 10;         const OP_SUB: u8 = 11;
const OP_MUL: u8 = 12;         const OP_DIV: u8 = 13;
const OP_IGUAL: u8 = 14;       const OP_DIF: u8 = 15;
const OP_MENOR: u8 = 16;       const OP_MAYOR: u8 = 17;
const OP_MENOR_IGUAL: u8 = 18; const OP_MAYOR_IGUAL: u8 = 19;
const OP_Y: u8 = 20;           const OP_O: u8 = 21;
const OP_NO: u8 = 22;          const OP_JUMP: u8 = 23;
const OP_JUMP_SI_FALSO: u8 = 24; const OP_LABEL: u8 = 25;
const OP_FN_DEF: u8 = 26;      const OP_CALL: u8 = 27;
const OP_RETURN: u8 = 28;      const OP_PRINT: u8 = 29;
const OP_READ: u8 = 30;        const OP_NEW_OBJ: u8 = 31;
const OP_SET_FIELD: u8 = 32;   const OP_GET_FIELD: u8 = 33;
const OP_CALL_METHOD: u8 = 34; const OP_ARRAY_NEW: u8 = 35;
const OP_ARRAY_GET: u8 = 36;   const OP_ARRAY_SET: u8 = 37;
...
#[derive(Clone)]
pub struct ObjetoDT { pub clase: String, pub campos: HashMap<String, ValorDT> }
#[derive(Clone)]
...
impl ValorDT {
    ...
    fn mostrar(&self) -> String {
        match self {
            ...
            ValorDT::Arreglo(e) => { let s: Vec<String> = e.iter().map(|v| v.mostrar()).collect(); format!("[{}]", s.join(",")) }
            ValorDT::Mapa(m) => { let s: Vec<String> = m.iter().map(|(k,v)| format!("\"{}\":{}", k, v.mostrar())).collect(); format!("{{{}}}", s.join(",")) }
        }
    }
}
...
pub fn compilar_bytecode(opcodes: &[crate::bytecode::Opcode]) -> BytecodeDT {
    ...
    for (_, op) in opcodes.iter().enumerate() {
        ...
        match op {
            ...
            crate::bytecode::Opcode::LoadIdx(idx) => { bc.code.push(OP_LOAD); bc.str_ops.push(format!("%idx_{}", idx)); }
            crate::bytecode::Opcode::StoreIdx(idx) => { bc.code.push(OP_STORE); bc.str_ops.push(format!("%idx_{}", idx)); }
            crate::bytecode::Opcode::DeclareIdx(idx, _) => { bc.code.push(OP_DECLARE); bc.str_ops.push(format!("%idx_{}", idx)); }
            // Opcodes fusionados: compilar como equivalentes no-fusionados
            crate::bytecode::Opcode::DeclareEnteroOp(idx, n) => {
                bc.code.push(OP_PUSH_ENTERO); bc.int_ops.push(*n);
                bc.code.push(OP_DECLARE); bc.str_ops.push(format!("%idx_{}", idx));
            }
            crate::bytecode::Opcode::DeclareBooleanoOp(idx, b) => {
                bc.code.push(OP_PUSH_BOOL); bc.int_ops.push(if *b { 1 } else { 0 });
                bc.code.push(OP_DECLARE); bc.str_ops.push(format!("%idx_{}", idx));
            }
            crate::bytecode::Opcode::StoreEnteroOp(idx, n) => {
                bc.code.push(OP_PUSH_ENTERO); bc.int_ops.push(*n);
                bc.code.push(OP_STORE); bc.str_ops.push(format!("%idx_{}", idx));
            }
            crate::bytecode::Opcode::Add => { bc.code.push(OP_ADD); }
            ...
            crate::bytecode::Opcode::Mayor => { bc.code.push(OP_MAYOR); }
            crate::bytecode::Opcode::MenorIgual => { bc.code.push(OP_MENOR_IGUAL); }
            crate::bytecode::Opcode::MayorIgual => { bc.code.push(OP_MAYOR_IGUAL); }
            crate::bytecode::Opcode::Y => { bc.code.push(OP_Y); }
            crate::bytecode::Opcode::O => { bc.code.push(OP_O); }
            crate::bytecode::Opcode::No => { bc.code.push(OP_NO); }
            crate::bytecode::Opcode::Jump(l) => { bc.code.push(OP_JUMP); bc.jump_ops.push(0); jump_placeholders.push((bc.jump_ops.len() - 1, *l)); }
            crate::bytecode::Opcode::JumpSiFalso(l) => { bc.code.push(OP_JUMP_SI_FALSO); bc.jump_ops.push(0); jump_placeholders.push((bc.jump_ops.len() - 1, *l)); }
            crate::bytecode::Opcode::Label(_) => { bc.code.push(OP_LABEL); }
            ...
            crate::bytecode::Opcode::ArrayLen => { bc.code.push(OP_ARRAY_LEN); }
            crate::bytecode::Opcode::MapNew(n) => { bc.code.push(OP_MAP_NEW); bc.int_ops.push(*n as i64); }
...
#[derive(Clone)]
struct FuncDT { ip: usize, param_names: Vec<String> }
...
impl ForjaDT {
    ...
    pub fn cargar_bytecode(&mut self, bc: BytecodeDT) {
        ...
        for (nombre, &(ip, ref params)) in &bc.fn_positions {
            self.funciones.insert(nombre.clone(), FuncDT { ip, param_names: params.clone() });
        }
    ...
    #[inline(always)] fn pop(&mut self) -> Result<ValorDT, ErrorDT> {
        self.stack.pop().ok_or(ErrorDT::StackUnderflow("pop".into()))
    }
    #[inline(always)] fn push(&mut self, v: ValorDT) { self.stack.push(v); }
                ...
                OP_CALL => {
                    ...
                    if let Some(func) = self.funciones.get(&nombre).cloned() {
                        ...
                        for (i, name) in func.param_names.iter().enumerate() {
                            let val = if i < args.len() { std::mem::replace(&mut args[i], ValorDT::Nulo) } else { ValorDT::Nulo };
                            ni.insert(name.clone(), i); nv.push(val);
                        }
                        self.variables.push(nv); self.var_indices.push(ni);
                        self.var_contadores.push(func.param_names.len());
                        self.ip = func.ip;
                    } else { return Err(ErrorDT::FuncionNoDefinida(nombre)); }
                }
                OP_RETURN => {
                ...
                OP_CALL_METHOD => {
                    ...
                    if let ValorDT::Objeto(obj_ref) = self.pop()? {
                        ...
                        if let Some(func) = self.funciones.get(&fn_name).cloned() {
                            ...
                            let mut ni = HashMap::with_capacity(func.param_names.len());
                            for (i, name) in func.param_names.iter().enumerate() {
                                let val = if i < all.len() { std::mem::replace(&mut all[i], ValorDT::Nulo) } else { ValorDT::Nulo };
                                ni.insert(name.clone(), i); nv.push(val);
                            }
                            self.variables.push(nv); self.var_indices.push(ni);
                            self.var_contadores.push(func.param_names.len());
                            self.ip = func.ip;
                        } else { return Err(ErrorDT::FuncionNoDefinida(fn_name)); }
    ...
    fn buscar_var(&self, nombre: &str) -> Result<&ValorDT, ErrorDT> {
        for (i, a) in self.var_indices.iter().enumerate().rev() {
            if let Some(&v) = a.get(nombre) {
                if let Some(val) = self.variables.get(i).and_then(|vars| vars.get(v)) { return Ok(val); }
            }
        }
    ...
    fn asignar_var(&mut self, nombre: &str, val: ValorDT) -> Result<(), ErrorDT> {
        for (i, a) in self.var_indices.iter().enumerate().rev() {
            if let Some(&v) = a.get(nombre) {
                if let Some(slot) = self.variables[i].get_mut(v) { *slot = val; return Ok(()); }
            }
        }
        Err(ErrorDT::VariableNoDeclarada(nombre.into()))
    }
...
enum BuiltinDT { Length, ToUpper, ToLower, Contains, Split, Trim, Reverse }
fn resolver_builtin_dt(metodo: &str) -> Option<BuiltinDT> {
    match metodo {
        "length" => Some(BuiltinDT::Length), "to_upper" => Some(BuiltinDT::ToUpper),
        "to_lower" => Some(BuiltinDT::ToLower), "contains" => Some(BuiltinDT::Contains),
        "split" => Some(BuiltinDT::Split), "trim" => Some(BuiltinDT::Trim),
        "reverse" => Some(BuiltinDT::Reverse), _ => None,
    }
}
impl ForjaDT {
    fn ejecutar_builtin_dt(&mut self, b: BuiltinDT, _n: usize) -> Result<(), ErrorDT> {
        match b {
            ...
            BuiltinDT::Split => { let sep = self.pop()?; match (self.pop()?, sep) { (ValorDT::Texto(s), ValorDT::Texto(sep)) => { let p: Vec<ValorDT> = s.split(sep.as_ref()).map(|p| ValorDT::Texto(Rc::from(p))).collect(); self.push(ValorDT::Arreglo(p)); } _ => return Err(ErrorDT::TipoIncompatible("split".into())) } }
            BuiltinDT::Trim => { match self.pop()? { ValorDT::Texto(s) => self.push(ValorDT::Texto(Rc::from(s.trim()))), _ => return Err(ErrorDT::TipoIncompatible("trim".into())) } }
            BuiltinDT::Reverse => { match self.pop()? { ValorDT::Texto(s) => { let r: String = s.chars().rev().collect(); self.push(ValorDT::Texto(Rc::from(r.as_str()))); } _ => return Err(ErrorDT::TipoIncompatible("reverse".into())) } }
        }
        Ok(())
    }
}

...
mod windows_impl {
    ...
    pub fn raw_mode(activar: bool) -> Result<(), String> {
        ...
        if h_in.is_null() {
            return Err("GetStdHandle falló".into());
        }
        ...
        if activar {
            ...
            if ret == 0 {
                return Err("GetConsoleMode falló".into());
            }
            unsafe {
                ORIG_MODE_IN = mode;
            }
            ...
            if ret == 0 {
                return Err("SetConsoleMode (raw) falló".into());
            }
            ...
            if unsafe { ORIG_MODE_IN } != 0 {
                unsafe {
                    SetConsoleMode(h_in, ORIG_MODE_IN);
                }
            }
            let h_out = unsafe { GetStdHandle(STD_OUTPUT_HANDLE) };
            if unsafe { ORIG_MODE_OUT } != 0 {
                unsafe {
                    SetConsoleMode(h_out, ORIG_MODE_OUT);
                }
            }
            Ok(())
        }
    ...
    pub fn tamano_terminal() -> Result<(i64, i64), String> {
        ...
        let ret =
            unsafe { GetConsoleScreenBufferInfo(h_out, &mut info as *mut ConsoleScreenBufferInfo) };
        if ret == 0 {
...
pub fn raw_mode(_activar: bool) -> Result<(), String> {
    #[cfg(unix)]
    {
        unix::raw_mode(_activar)
    }
    #[cfg(windows)]
    {
        windows_impl::raw_mode(_activar)
    }
...
pub fn tamano_terminal() -> Result<(i64, i64), String> {
    #[cfg(unix)]
    {
        unix::tamano_terminal()
    }
    #[cfg(windows)]
    {
        windows_impl::tamano_terminal()
    }
    #[cfg(target_arch = "wasm32")]
    {

...
fn test_error_tipo_error_lexico() {
    let err = ErrorForja::new(ErrorTipo::ErrorLexico, 1, 1, "caracter invalido", "eliminalo");
    assert_eq!(err.tipo, ErrorTipo::ErrorLexico);
}

#[test]
fn test_error_tipo_error_sintactico() {
    let err = ErrorForja::new(ErrorTipo::ErrorSintactico, 2, 5, "se esperaba ';'", "agrega ;");
    assert_eq!(err.tipo, ErrorTipo::ErrorSintactico);
}

#[test]
fn test_error_tipo_error_semantico() {
    let err = ErrorForja::new(ErrorTipo::ErrorSemantico, 3, 10, "variable no declarada", "declarala");
    assert_eq!(err.tipo, ErrorTipo::ErrorSemantico);
}

#[test]
fn test_error_tipo_error_de_tipo() {
    let err = ErrorForja::new(ErrorTipo::ErrorDeTipo, 4, 15, "tipos incompatibles", "usa el tipo correcto");
    assert_eq!(err.tipo, ErrorTipo::ErrorDeTipo);
}

#[test]
fn test_error_tipo_error_interno() {
    let err = ErrorForja::new(ErrorTipo::ErrorInterno, 5, 20, "error interno", "reporta el bug");
    assert_eq!(err.tipo, ErrorTipo::ErrorInterno);
}

#[test]
fn test_error_tipo_limite_archivo() {
    let err = ErrorForja::new(ErrorTipo::LimiteArchivo { ruta: "x.fa".into(), max: 10, actual: 20 }, 1, 1, "demasiado grande", "");
    assert_eq!(err.tipo, ErrorTipo::LimiteArchivo { ruta: "x.fa".into(), max: 10, actual: 20 });
}
...
fn test_error_campos() {
    let err = ErrorForja::new(ErrorTipo::ErrorSintactico, 10, 15, "mensaje de error", "sugerencia util");
...
fn test_error_formato_basico() {
    let err = ErrorForja::new(ErrorTipo::ErrorSintactico, 5, 3, "Error de sintaxis", "Revisa la sintaxis");
...
fn test_error_sugerencia_incluida() {
    let err = ErrorForja::new(ErrorTipo::ErrorSemantico, 1, 1, "variable no encontrada", "declara la variable primero");
    let texto = format!("{}", err);
    // Remove ANSI codes before checking
    let clean = texto.replace("\x1b[", "");
    assert!(clean.contains("declara la variable primero") || clean.contains("variable no encontrada"));
}
